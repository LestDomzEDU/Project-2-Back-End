...
public class EventController {
  ...
  @GetMapping("/events") public List<Map<String,Object>> listEvents() {
    return jdbc.queryForList("""
      SELECT id, league, home_team, away_team, start_time, status, result
      FROM events ORDER BY start_time ASC
    """);
  }
  @GetMapping("/events/{id}") public Map<String,Object> getEvent(@PathVariable long id) {
    ...
    List<Map<String,Object>> odds = jdbc.queryForList("""
      ...
      WHERE m.event_id = ? AND m.type = 'MONEYLINE' ORDER BY o.selection
    """, id);
    event.put("odds", odds); return event;
  }
  @GetMapping("/events/{id}/odds") public List<Map<String,Object>> odds(@PathVariable long id){
    return jdbc.queryForList("""
      SELECT o.id, o.selection, o.american, o.`decimal` AS decimal
      FROM markets m JOIN odds o ON o.market_id = m.id
      WHERE m.event_id = ? AND m.type = 'MONEYLINE' ORDER BY o.selection
